...
DIRREF directory_init (const char *dirpath) {
	...
	// if the path points to a symbolic link, the _WIN32_FIND_DATA buffer contains
...
char *directory_read (DIRREF ref, char *win32buffer) {
	...
	while (1) {
		...
		// cFileName from _WIN32_FIND_DATAA is a fixed size array, and findData is local
